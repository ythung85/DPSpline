...
def ECM(par, initial_xi = 1, initial_sigma = 1, initial_lambda = 1e-4):
	...
	for l in range(n_block):
		...
		for i in range(num_neurons):
			Ncov = (Cov_a - (Cov_a @ flatB[i]) @ (torch.linalg.solve(flatB[i].T @ Cov_a @ flatB[i] + Cov_e[size*i:size*(i+1),size*i:size*(i+1)], flatB[i].T @ Cov_a)))
			Nmu = (Cov_a @ flatB[i]) @ (torch.linalg.solve(flatB[i].T @ Cov_a @ flatB[i] + Cov_e[size*i:size*(i+1),size*i:size*(i+1)], By[:,i].reshape(-1,1)))
...
def ECM_layersise_update(model, par, Lambda, x, y):
	...
	for l in range(n_layer):    
		...
		for i in range(nm):
			...
			NW[:, i] = (torch.linalg.solve(BB.T @ BB + (Lambda[l]/Size[l]) * (DB.T @ DB), BB.T @ B1y))
...
class StackBS_block(nn.Module):
	def __init__(self, block, degree, num_knots, num_neurons, num_blocks, dropout = 0.0, bias = True):
		...
		self.model = nn.ModuleDict({
			f'block_{i}': block(degree = degree, num_knots = num_knots, num_neurons = num_neurons, dropout = dropout)
...
if __name__ == "__main__":
	...
	for d in range(ndf):
		...
		for epoch in range(1, fine_tune_epoch):
			...
			train_loss = train_one_epoch(DeepPS, X_train, y_train, criterion, optimizer, device, grad = False)
			for l in range(nl):
				train_loss += (BestLambda[l]/n * torch.norm(diag_mat_weights(DPS_Wstack[l].size()[0]).to(device) @ DPS_Wstack[l]))
		...
		DeepPS = DPS(input_dim = ndim, degree = 3, num_knots = nk, num_neurons = nm, num_bsl = nl, dropout = dp, output_dim = Fout, bias = True).to(device)
		DeepPS.load_state_dict(torch.load('./best_DPS_model.pt', weights_only = True))
		DeepPS.eval()
		with torch.no_grad():
	...
	print('DNN...')
	nneuron = 50
	model = FlexibleMLP([ndim, nneuron, nneuron, 1], activation_fn=nn.ReLU, output_activation = None).to(device)
	...
	for d in range(ndf):
		...
		DNN = FlexibleMLP([ndim, nneuron, nneuron, 1], activation_fn=nn.ReLU, output_activation = None).to(device)
	...
	# Command: python3 Sparse.py --data A --trainsize 400 --testsize 200 --Fin 10 --Fout 1 --nk 10 --nm 50 --rep 2 --nl 1 --lr 1e-2 --nepoch 100 --fine_tune_epoch 1000 #